...
if(isset($_POST['submit'])){
    ...
    if(!empty($email) && !empty($name) && !empty($subject) && !empty($message)){
        ...
        if(filter_var($email, FILTER_VALIDATE_EMAIL) === false){
            $PHP_Email_Form = 'Please enter your valid email.';
            $msgClassk = 'errordiv';
        }else{
            ...
            if(mail($toEmail,$emailSubject,$htmlContent,$headers)){
                $PHP_Email_Form = 'Pesan Anda sudah terkirim dengan sukses !';
                $msgClass = 'succdiv';
            }else{
                $PHP_Email_Form = 'Maaf pesan Anda gagal terkirim, silahkan ulangi lagi.';
                $msgClass = 'errordiv';
            }
        }
    }else{
        $PHP_Email_Form = 'Harap mengisi semua field data';
        $msgClass = 'errordiv';
    }
}
?>